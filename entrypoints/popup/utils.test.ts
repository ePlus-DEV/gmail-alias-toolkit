...
describe('generateAlias', () => {
  it('generates alias with plus tag', () => {
    expect(generateAlias('user@gmail.com', 'shopping')).toBe('user+shopping@gmail.com');
  });
  ...
  it('preserves dots in username', () => {
    expect(generateAlias('first.last@gmail.com', 'work')).toBe('first.last+work@gmail.com');
  });

  it('handles googlemail domain', () => {
    expect(generateAlias('user@googlemail.com', 'test')).toBe('user+test@googlemail.com');
  });

  it('handles hyphenated tags', () => {
    expect(generateAlias('user@gmail.com', 'private-mail-abc1')).toBe('user+private-mail-abc1@gmail.com');
  });
});
...
describe('validateEmail', () => {
  it('rejects empty string', () => {
    expect(validateEmail('')).toMatchObject({ isValid: false, error: 'Email is required' });
  });
  ...
  it('accepts googlemail domain', () => {
    expect(validateEmail('user@googlemail.com')).toMatchObject({ isValid: true });
  });

  it('accepts dotted gmail username', () => {
    expect(validateEmail('first.last@gmail.com')).toMatchObject({ isValid: true });
  });
